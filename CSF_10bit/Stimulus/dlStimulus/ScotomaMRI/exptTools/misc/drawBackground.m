...
if ~exist('eye','var') || isempty(eye)
    eye = 1;
end

if isfield(d,'horizontalOffset')
    xPos = d.fixX + (2*eye-1)*d.horizontalOffset;
else
    xPos = 0;
end

Screen('DrawTexture', d.windowPtr, stimulus.backgroundTexture, [],...
    CenterRectOnPointd([-d.numPixels(1),-d.numPixels(1),d.numPixels(1),d.numPixels(1)],xPos,d.fixY), 2*(eye-.5)*d.screenRotation);
