function [stimulus otherSequence] = makeRetinotopyStimulus_dotbars(params)
...
% if ~exist(params.stereoMode, 'var')
%     stimulus.stereoMode = 0;
% end
...
%         
%         dotbarX(:,:,imgNum) = x(window);
%         dotbarY(:,:,imgNum) = y(window);
%         dotbarRad(imgNum) = outerRad;
%     end
%             
%             
...
%                         noysSlope = 1.0; %1.5;
%                         checks(:,:,ii)=minCmapVal+ceil((maxCmapVal-minCmapVal) * oneoverf(noysSlope,size(wedges,1),size(wedges,2))); % one over f texture
%                     end;
% 
...
% minsec = fixationChangeTime./duration.stimframe; % Determine length of fixationChangeTime (2 seconds) in frames
% fixSeq = ones(minsec,1)*round(rand(1,ceil(length(sequence)/minsec))); % Create sequence of random 6 second epochs
% fixSeq = fixSeq(:)+1; % Go from [0,1] to [1,2] range
% fixSeq = fixSeq(1:length(sequence)); % Truncate to experiment length
% % force binary
% fixSeq(fixSeq>2)=2; 
% fixSeq(fixSeq<1)=1;

% Dot sequence for depth discrimination task
minsec = fixationChangeTime./duration.stimframe; % Determine length of fixationChangeTime (2 seconds) in frames
reds = [ones(minsec/4,1); 3.*ones(3*minsec/4,1)];
greens = [2.*ones(minsec/4,1); 3.*ones(3*minsec/4,1)];
blues = 3.*ones(minsec,1);
fixSeq = [];
for ii = 1:ceil(length(sequence)/minsec)
    colors = floor(3*rand);
    if colors < 1
        fixSeq = [fixSeq; reds];
    elseif colors < 2
...
% stimulus.dotbarX = dotbarX;
% stimulus.dotbarY = dotbarY;
% stimulus.dotbarRad = dotbarRad;
% nSteps = length(sequence)./length(orientations);
% orientations = repmat(orientations(:)',nSteps,1);
