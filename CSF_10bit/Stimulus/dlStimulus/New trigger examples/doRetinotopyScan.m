...
try,
    ...
    switch params.experiment
        ...
        case {'Scotoma stimulus','Vertical motion','Scotoma fixation'}
...
%             disp(sprintf('[%s]:Fixation dot task: percent correct: %.1f %%',mfilename,pc));
            ...
            pc = []; diffpos = [];
            diffpos = unique(data(~isnan(data(:,1)),[5 6]),'rows');
            
            for ii = 1:size(diffpos,1)
               iis = data(:,5)==diffpos(ii,1) & data(:,6)==diffpos(ii,2);
               
               correctresp = data(iis,2)==65 & data(iis,4)==-1 |...
                             data(iis,2)==68 & data(iis,4)==1;
               pc(ii) = sum(correctresp)./sum(iis);
            end
        ...
        elseif any(strcmp(params.experiment,{'Scotoma fixation'}))
            [params,params.display,response,data, quitProg] = showScotomaStimulusFixation(params, params.display, stimulus, time0);%, dotparams, duration, stimdots, BGdots, BGdotColors);
        elseif any(strcmp(params.experiment,{'Localizer'}))
            ...
            if exist(['~/Documents/MATLAB/ScotomaMRI/Displays/' params.calibration '/'],'dir')
                save(['~/Documents/MATLAB/ScotomaMRI/Displays/' params.calibration '/screenRotation.mat'],'-STRUCT','display','screenRotation','horizontalOffset');
            end
