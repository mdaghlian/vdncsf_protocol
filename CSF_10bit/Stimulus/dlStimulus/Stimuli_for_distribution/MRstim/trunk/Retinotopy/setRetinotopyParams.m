...
expNames = {'rotating wedge (45deg duty)','rotating wedge (90deg duty)','rotating wedge (45deg duty) Reverse', ...
			...
			'full-field, on-off', 'full-field, on-off (impulse)','Translating Bars 8 Pass (Dumoulin)',...
            'Translating Bars 8 Pass (contrast)'};
...
case {'Translating Bars 8 Pass (Dumoulin)'},
    ...
    params.insertBlanks.do = 1;
    params.numSubRings = (params.radius-params.innerRad)/(params.radius); 
    params.blanksAfterPass = 2; % 0 (false) puts blanks within the bar pass. 1 (True) puts blanks after bars have passed, every pass. 2 puts blanks after only diagonal bar passes.
case {'Translating Bars 8 Pass (contrast)'},
    params.type = 'bar';
	params.ringDeg = params.radius./4; 
	params.seqDirection = 0;
    params.insertBlanks.do = 1;
    params.contrastCondition = round(127*(logspace(log10(1),log10(100),6))/100)
    params.contrast = inputdlg('Select contrast condition [1-3-8-20-51-127]');
    params.contrast = params.contrastCondition(str2num(params.contrast{1}));
    params.numSubRings = (params.radius-params.innerRad)/(params.radius); 
    params.blanksAfterPass = 2; % 0 (false) puts blanks within the bar pass. 1 (True) puts blanks after bars have passed, every pass. 2 puts blanks after only diagonal bar passes.
otherwise,
