...
% 2018.07.25 JD: changed so that blanksAfterPass now gives the traditional
% order of the 8 bar passes. Edited around Ln 281. Also added if statement
% in Ln 23. Added double(uint8 statement around numBarImages
% Ln 209 so that it takes the right amount of elements from img

if isfield(params,'blanksAfterPass')
   blanksAfterPass = params.blanksAfterPass; 
else
   blanksAfterPass=0;  %1 (True) puts blanks after bars have passed, every pass. 0 (false) puts blanks within the bar pass. 2 puts blanks after only horizontal bar passes, which is what we do
end

blankTime=16;       %Blank time (in seconds) will be rounded up to the nearest TR
...
if blanksAfterPass==2;
    ...
    numBarImages=double(uint8( params.numImages-(totalBlankTime./params.tr)) );
...
%    disp(sprintf('[%s]:size stimulus: %dx%d pixels.',mfilename,n,m));
    ...
    orientations = orientations([1 6 3 8 5 2 7 4]);%[3 2 5 4 7 6 1 8]);  %[6 3 8 5 2 7 4 1]);
    ...
    switch params.experiment,
        case {'8 bars (LMS) with blanks','Translating Bars 8 Pass (contrast)'}
    ...
    for imgNum=1:halfNumImages
        ...
        % yet another loop to be able to move the checks...
                ...
                idx_img = (imgNum-1).*numMotSteps+1:imgNum.*numMotSteps;
                images(:,:,idx_img) = uint8(img(:,:,1:length(idx_img)));
...
%         sequence=cat(1,sequence(1:blankCycleFrames), blankInsert, sequence(blankCycleFrames+1:3*blankCycleFrames), blankInsert, sequence(blankCycleFrames*3+1:5*blankCycleFrames),blankInsert, sequence(blankCycleFrames*5+1:7*blankCycleFrames), blankInsert, sequence(blankCycleFrames*7+1:end));
        sequence=cat(1,sequence(1:blankCycleFrames*2), blankInsert, sequence(blankCycleFrames*2+1:4*blankCycleFrames), blankInsert, sequence(blankCycleFrames*4+1:6*blankCycleFrames),blankInsert, sequence(blankCycleFrames*6+1:end), blankInsert);
...
switch params.experiment,
    case {'Translating Bars 8 Pass (contrast)'}
        images(images==255)=128+params.contrast;
        images(images==  0)=128-params.contrast;
        % Condities (127 = 100% contrast)
        % 127*(logspace(log10(1),log10(100),6))/100
    otherwise,
        % do nothing
end;
