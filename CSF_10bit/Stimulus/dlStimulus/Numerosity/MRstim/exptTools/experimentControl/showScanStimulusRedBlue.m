function [response, timing, quitProg] = showScanStimulusRedBlue(display,stimulus, t0)
...
highestColormapOneValue=max(stimulus.seq(:))./2;
...
for frame = 1:nFrames	
    ...
    if stimulus.seq(frame)>0
        % put in an image
        if frame>1
            if stimulus.seq(frame)>highestColormapOneValue && stimulus.seq(frame-1)<=highestColormapOneValue
                Screen('LoadNormalizedGammaTable', display.windowPtr, stimulus.cmap(:,:,2));
            elseif stimulus.seq(frame)<=highestColormapOneValue && stimulus.seq(frame-1)>highestColormapOneValue
                Screen('LoadNormalizedGammaTable', display.windowPtr, stimulus.cmap(:,:,1));
            end
        else
            Screen('LoadNormalizedGammaTable', display.windowPtr, stimulus.cmap(:,:,1)); 
        end
