function [response, timing, quitProg, success] = showScanStimulusAttentionBar(display,params,stimulus, t0)
...
StartingPoint=alphaLevelBar;   %Starting point of staircase, a guess of performance
...
alphaLevelBar = QuestQuantile(q)
if alphaLevelBar>0.75;
    alphaLevelBar=0.75;
elseif alphaLevelBar<0
    alphaLevelBar=0;
end
responseGiven=0;
...
%         end
        correct=params.psychResponse(whichTR)==params.randomIntervalBar(whichTR)
        q=QuestUpdate(q,alphaLevelBar,correct);
...
save('savesBar','saveResponses','saveAlphas','saveQuantiles');

correctResponses=100*(sum(params.randomIntervalBar(barPeriods)==params.psychResponse(barPeriods))./length(barPeriods));
disp(sprintf('[%s]:Bar psychophysics task percentage correct: %f' ,mfilename, correctResponses));
