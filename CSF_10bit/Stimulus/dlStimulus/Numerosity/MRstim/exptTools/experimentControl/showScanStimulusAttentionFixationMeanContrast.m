function [response, timing, quitProg, params] = showScanStimulusAttentionFixationMeanContrast(display,params,stimulus, t0)
...
alphaLevelFixation=maxContrastFixation/2;

meanContrastBar=0.25;
alphaLevelBar=0.25;
blendBar = 1;
blendFix = 1;
eyeTracker=0;
...
if alphaLevelFixation>maxContrastFixation/2;
    alphaLevelFixation=maxContrastFixation/2;
...
for frame = 1:nFrames
        ...
        if blendBar ==1;
            if params.randomIntervalBar(whichTR) == 1 
                if whichFrame <= params.StimOnFrames;
                    Screen('DrawTexture', display.windowPtr, stimulus.textures(length(stimulus.textures)), stimulus.srcRect, stimulus.destRect, [], [], meanContrastBar+alphaLevelBar);
        ...
        if ismember(whichTR,barPeriods)
            if whichFrame <= params.StimOnFrames || (whichFrame > (params.StimOnFrames + params.ISIframes) && whichFrame <= (2*params.StimOnFrames + params.ISIframes));
                ...
                if blendFix ==1;
                    ...
                    %                     Screen('DrawTexture', display.windowPtr, stimulus.textures(length(stimulus.textures)), stimulus.srcRect,[display.fixX-horSize display.fixY-verSize display.fixX+horSize display.fixY+verSize], [], [], alphaLevel);
                    %                 end
                    if params.randomIntervalFixation(whichTR) == 1
                        if whichFrame <= params.StimOnFrames;
                            Screen('DrawTexture', display.windowPtr, stimulus.alphaTexture, stimulus.srcRect,stimulus.destRect, [], [], meanContrastFixation+alphaLevelFixation);
                        elseif whichFrame > (params.StimOnFrames + params.ISIframes) && whichFrame <= (2*params.StimOnFrames + params.ISIframes)
                            Screen('DrawTexture', display.windowPtr, stimulus.alphaTexture, stimulus.srcRect,stimulus.destRect, [], [], meanContrastFixation-alphaLevelFixation);
                        end
                    elseif params.randomIntervalFixation(whichTR)==2
                        if whichFrame <= params.StimOnFrames;
                            Screen('DrawTexture', display.windowPtr, stimulus.alphaTexture, stimulus.srcRect,stimulus.destRect, [], [], meanContrastFixation-alphaLevelFixation);
                        elseif whichFrame > (params.StimOnFrames + params.ISIframes) && whichFrame <= (2*params.StimOnFrames + params.ISIframes)
                            Screen('DrawTexture', display.windowPtr, stimulus.alphaTexture, stimulus.srcRect,stimulus.destRect, [], [], meanContrastFixation+alphaLevelFixation);
    ...
    while(waitTime<0),
        ...
        if display.devices.UMCport==2
            ...
            if ssKeyCode(1)==66 || ssKeyCode(1)==65
                params.psychResponse(whichTR)=2;
                responseGiven=1
            elseif ssKeyCode(1)==67 || ssKeyCode(1)==68
...
correctResponses=100*(sum(params.randomIntervalFixation(barPeriods)==params.psychResponse(barPeriods))./length(barPeriods));
correctResponses2=100*(sum(sum(respAndInt(2,:,:)))/(numStairsFinished*staircaseLength));
disp(sprintf('[%s]:Fixation psychophysics whole task percentage correct: %f' ,mfilename, correctResponses));
disp(sprintf('[%s]:Fixation psychophysics task finished staircases only percentage correct: %f' ,mfilename, correctResponses2));
% that's it
ShowCursor;
timing = GetSecs-t0;
% t=QuestMean(q);		
