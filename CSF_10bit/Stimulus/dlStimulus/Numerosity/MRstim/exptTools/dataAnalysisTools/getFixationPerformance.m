...
if ~isfield(fixParams,'responseTime'),
    fixParams.responseTime = [0.01 1.8];  % look for responses between .01 and 3 seconds after
                                        % stimulus change
end
...
switch fixParams.task,
    case 'Detect fixation change',
        ...
        resp     = find(diff(response.keyCode(:,1))>0);     % any response?
    ...
    otherwise,
        fprintf(1,'[%s]:Unknow fixation task\n',mfilename);
end
