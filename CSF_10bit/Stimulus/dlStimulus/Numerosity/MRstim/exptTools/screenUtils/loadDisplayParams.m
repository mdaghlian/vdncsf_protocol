...
% default i an M-file, 'displayParams.m', which defines some of the key
...
try
    ...
    if displayID.bitsPerPixel==8 || displayID.bitsPerPixel==24 || displayID.bitsPerPixel==32
        displayID.maxRgbValue = 1; %65535; %1023; % 255
        % We reserve the first and last gamma table entry for stuff like
        % the fixation point.
        displayID.stimRgbRange = [0,1]; %[1,65535]; % 1024 % 254
        displayID.backColorIndex = 0.5; %32768; %512; % 128
    else
        error('Unknown bitPerPixel value!');
    end

    % Set a default background- users will probably want to over ride this.
    displayID.backColorRgb = 0.5; %[repmat(round(displayID.maxRgbValue/2),1,3)];
    displayID.textColorRgb = 1; %repmat(displayID.maxRgbValue,1,3);
    displayID.fixType = 'disk';
    displayID.fixSizePixels = 3;
    displayID.fixColorRgb = 1; %[displayID.maxRgbValue 0 0];
...
%         %disp(displayPath);
%         fprintf(1,'[%s]:Loaded gamma table.\n',mfilename);
%     else
%         gammaCol = (0:displayID.maxGammaValue)';
%         displayID.gammaTable = [gammaCol gammaCol gammaCol];
%         fprintf(1,'[%s]:Gamma table not found.  Using linear gamma table instead.\n',mfilename);
%     end
% 
%     % create a normalized gamma table
%     displayID.gamma = displayID.gammaTable./(size(displayID.gammaTable,1)-1);

    % Load spectra
    if(exist(fullfile(displayPath,'spectra.mat'),'file'))
        load spectra
        if ~exist('monitorSpectra','var')
            fprintf(1,'[%s]:Spectra not in spectra file (only important for LMS-specified stimuli)\n',mfilename);
        else
            displayID.spectra = monitorSpectra(:,1:3);
            fprintf(1,'[%s]:Loaded spectra.\n',mfilename);
        end
    else
        fprintf(1,'[%s]:Spectra table not found (only important for LMS-specified stimuli).\n',mfilename);
    end
    chdir(homedir);

    % Check the frame rate
    if max(Screen('screens')) < displayID.screenNumber,
        fprintf(1,'[%s]:resetting screenNumber %d -> %d.\n',mfilename,...
