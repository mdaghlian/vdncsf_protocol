function images=preprocessImages(path)
...
for i=1:length(files)
    ...
    images(i).image=imread(images(i).filename);
    dimensions=size(images(i).image);
    %resize image to fit circle matrix
    if dimensions(2)>dimensions(1) %images in lanscape aspect
        if dimensions(1)~= 321
            images(i).image=imresize(images(i).image, [321 NaN]);
        end
    else
        if dimensions(2)~=321
            images(i).image=imresize(images(i).image, [NaN 321]);
        end
    end
    images(i).grayscale=single(rgb2gray(images(i).image)); 
    dimensions=size(images(i).image);
    ...
    if segpresent>0
        ...
        images(i).inputseg=segline.*51;
    else
        [mean, segline, avesd]=LocalStat(images(i).grayscale, 1);
        images(i).inputseg=(segline./max(max(segline))).*255;
    end
    
    segtmp=images(i).inputseg./510;
    ...
    if dimensions(2)>dimensions(1);
        ...
        centrey=single(161);
        meaner=images(i).grayscale(bii);
        meaner=sum(meaner)/length(meaner);
        circleimage=single((((1-images(i).grayscale).*(0.5/(1-meaner)))-0.5).*bestcirclemask+0.5);
        images(i).circleimage=circleimage(:, centrex-160:centrex+160).*256;
    elseif dimensions(1)>dimensions(2);
        ...
        meaner=sum(meaner)/length(meaner);
        circleimage=single((((1-images(i).grayscale).*(0.5/(1-meaner)))-0.5).*bestcirclemask+0.5);
        images(i).circleimage=circleimage(centrey-160:centrey+160, :).*256;
    elseif dimensions(2)==dimensions(1)
        bii=circlemask==1;
        meaner=images(i).grayscale(bii);
        meaner=sum(meaner)/length(meaner);
        circleimage=single((((1-images(i).grayscale).*(0.5/(1-meaner)))-0.5).*circlemask+0.5);
        images(i).circleimage=circleimage(centrey-160:centrey+160, :).*256;
        
    end 
    imwrite(images(i).circleimage, gray(256), strcat(path, '/X', files(i).name), 'jpg', 'Bitdepth', 8);
