function im=synthetic_contours_edge(sizeInPix,sizeInDeg,contourWidthInDeg,orientationInDeg,orientationBPinDeg,circularWindow,region)
% im = synthetic_contours_edge(sizeInPix,sizeInDeg,contourWidthInDeg,orientationBPinDeg,region);
%
...
% filter image
[im] = filterimagecontours(imr,single(oriFilter),single(1),[],orientationInDeg);

% different filter inside region
if ~isempty(region)
    % rotate region
    rmask = region.mask;
    im2   = ones(sizeInPix).*-1;
...
%     % blend
      im  = im.*(mask2) + im2.*(1-mask2);
end

% binary
im = sign(im).*(sign(rand(1)-0.5));
