function [out,imaginary,pow] = randphase2(matrixn,rand_orient,padd,mask,nphaseshifts,doPlot);
% RANDPHASE - phase (and orientation) scrambling
% [out,imag,pow] = randphase(matrix,rand_orient,padd,mask,nphaseshifts,doPlot);
...
% ---------------------------------
% 22/9/11 - edit by TvK expanding it for k matrices that get the same
% random orientation/phase scrambling

nrofm=size(matrixn,3);
...
for k=1:nrofm
    ...
    [len,wid]=size(matrix);
    if len ~= wid,
       disp(sprintf('Error -matrix %d -- not a square matrix',k));
    end;
       ...
       for n=0:round(max(dist_from_center(:))),
          ...
          if length(target_circle) == 1,
            new_pow(target_circle)=mean(pow(target_circle));
              ...
              %this is a workaround to eliminate variance between the two
              %matrices
             if k==1,
                 C{n}=repmat(rand(length(target_circle)/2,1)+.5,2,1);
             end
            new_pow(target_circle)=mean(pow(target_circle)).*C{n};
    ...
    imaginary(:,:,k)=imag(newmatrix(1:len,1:wid));
    out(:,:,k)=real(ifft2(ifftshift(newmatrix),padd,padd))+mean_matrix;
