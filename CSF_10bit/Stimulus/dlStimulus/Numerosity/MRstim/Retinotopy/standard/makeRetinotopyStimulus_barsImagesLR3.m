function [stimulus] = makeRetinotopyStimulus_barsImagesLR3(params)
...
%if isempty(retinotopimg)
    disp(sprintf('[%s]: loading natural images (516x516).',mfilename));
    switch params.experiment,
        case {'Retinotopy Images Cow (lr 3)'}
            load('/Users/lab/Documents/MATLAB/RetinotopyImages/cowimg(538).mat','-mat');
        case {'Retinotopy Images Natural (lr 3)'}
        switch params.NatVPhs
            case {1}
            %load('/Users/lab/Documents/MATLAB/RetinotopyImages/retinotopimg_new.mat','-mat');
                 load('H:\Serges_Group\Akhil\CB_20170717\pRF\retinotopimg_new.mat','-mat');
            case {2}
                load('H:\Serges_Group\Akhil\CB_20170717\pRF\retinotopimg_PhSc_1.mat','-mat');
                retinotopimg = retinotopimg_PhSc;
            case {3}
                load('H:\Serges_Group\Akhil\CB_20170717\pRF\retinotopimg_PhSc_2.mat','-mat');
                retinotopimg = retinotopimg_PhSc;
            case {4}
                load('H:\Serges_Group\Akhil\CB_20170717\pRF\retinotopimg_PhSc_3.mat','-mat');
                retinotopimg = retinotopimg_PhSc;
            case {5}
                load('H:\Serges_Group\Akhil\CB_20170717\pRF\retinotopimg_PhSc_4.mat','-mat');
                retinotopimg = retinotopimg_PhSc;
            case {6}
                load('H:\Serges_Group\Akhil\CB_20170717\pRF\retinotopimg_PhSc_5.mat','-mat');
                retinotopimg = retinotopimg_PhSc;
        end
    end
...
%    disp(sprintf('[%s]:size stimulus: %dx%d pixels.',mfilename,n,m));
   ...
   outerRad = params.radius;
    ...
    halfNumImages = numBarImages;%params.numImages;
    ...
    m=516;%angle2pix(params.display,2*outerRad); 
    n=516;%angle2pix(params.display,2*outerRad);
    ...
    for imgNum=1:numBarImages
        ...
        for ii=1:3,
            ...
            while rindex==prevInd%~isempty( intersect( rindex, prevInd ) )
                rindex = ceil( rand() * length( retinotopimg ) );
            end
...
%         end
%     end
    motionSeq = [ones(12,1); ones(3,1)+9; ones(12,1)+1; ones(3,1)+9 ];
