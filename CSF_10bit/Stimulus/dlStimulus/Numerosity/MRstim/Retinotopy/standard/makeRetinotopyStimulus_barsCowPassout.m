function [stimulus otherSequence] = makeRetinotopyStimulus_barsCowPassout(params)
...
%    disp(sprintf('[%s]:size stimulus: %dx%d pixels.',mfilename,n,m));
    ...
    stimSize=1800;%n+ceil(framesDuration*speedPixPerFrame*2);
    bigCowPattern=uint8(round(bandpassimage(rand(stimSize), [angle2pix(params.display,0.5) angle2pix(params.display,1)],1)));
    ...
    for imgNum=1:numBarImages
        ...
        if remake_xy(imgNum) >=0,
            ...
            switch params.experiment
                ...
                case {'8 bars with blanks Cow (0) Slow','8 bars with blanks Cow (0) Medium','8 bars with blanks Cow (0) Fast',...
                      '8 bars with blanks Cow (90) Slow', '8 bars with blanks Cow (90) Medium','8 bars with blanks Cow (90) Fast','8 bars with blanks Cow (Flicker) Slow','8 bars with blanks Cow (Flicker) Fast'}   
...
sequence = logical(1-cat(3, windows(:,:,size(windows,3)+1-params.prescanDuration/params.tr:size(windows,3)),windows));
