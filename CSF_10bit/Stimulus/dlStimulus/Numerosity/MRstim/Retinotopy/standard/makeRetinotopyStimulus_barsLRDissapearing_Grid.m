function [stimulus otherSequence] = makeRetinotopyStimulus_barsLR3(params)
...
%    disp(sprintf('[%s]:size stimulus: %dx%d pixels.',mfilename,n,m));
    ...
    orientations = orientations([1 1 1 1 1 1 1 1]);  %[6 3 8 5 2 7 4 1]);
...
%Insert blanks
sequenceIn=sequence;
sequence(15*30+1:16*30)=size(images,3);
sequence(24*30+1:32*30)=size(images,3);
sequence(46*30+1:48*30)=size(images,3);
sequence(57*30+1:64*30)=size(images,3);
sequence(77*30+1:80*30)=size(images,3);
sequence(90*30+1:96*30)=size(images,3);
sequence(108*30+1:112*30)=size(images,3);
sequence(123*30+1:128*30)=size(images,3);
...
sequence = [sequenceIn(length(sequence)+1-duration.prescan.stimframes:end); sequence];
