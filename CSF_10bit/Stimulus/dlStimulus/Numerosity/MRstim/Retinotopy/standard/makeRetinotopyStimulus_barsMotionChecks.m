function [stimulus otherSequence] = makeRetinotopyStimulus_barsMotionChecks(params)
...
switch params.experiment
    case {'8 bars with blanks Checks Together (90) 2.5d/s','8 bars with blanks Checks Together (90) 5d/s'};
        sineChecks=0;
        softEdgeWindow=0;
    case {'8 bars with blanks Sine (90) 1.5d/s', '8 bars with blanks Sine (90) 2.5d/s', '8 bars with blanks Sine (90) 3.75d/s', '8 bars with blanks Sine (90) 5d/s', '8 bars with blanks Sine (90) 7.5d/s', '8 bars with blanks Sine (0) 2.5d/s', '8 bars with blanks Sine (0) 5d/s', '8 bars with blanks Sine (90) 1.5d/s 20%','8 bars with blanks Sine (90) 2.5d/s 20%','8 bars with blanks Sine (90) 3.75d/s 20%', '8 bars with blanks Sine (90) 5d/s 20%','8 bars with blanks Sine (90) 7.5d/s 20%'};
        sineChecks=1;
        softEdgeWindow=1;
    otherwise,
        sineChecks=0;
...
%    disp(sprintf('[%s]:size stimulus: %dx%d pixels.',mfilename,n,m));
    ...
    for imgNum=1:halfNumImages
        ...
        if remake_xy(imgNum) >=0,
            ...
            switch params.experiment
                case {'8 bars with blanks Checks Counter (90) 2.5d/s', '8 bars with blanks Checks Counter (90) 5d/s'};
                    ...
                    for ii=1:numMotSteps,
                        if sineChecks==1
                            tmprings1 = (((cos(y*numSubRings*(2*pi/ringWidth)+(ii-1)/numMotSteps*2*pi)+1)/2));
                            tmprings2 = (((cos(y*numSubRings*(2*pi/ringWidth)-(ii-1)/numMotSteps*2*pi)+1)/2));
                        else
                        ...
                        checks(:,:,ii)=(fliplr(rings'));
%                         if sineChecks==1
%                             checks(:,:,ii)=minCmapVal+ceil((maxCmapVal-minCmapVal) * (fliplr(rings')));
%                         else
%                             checks(:,:,ii)=minCmapVal+ceil((maxCmapVal-minCmapVal) * (fliplr(wedges').*fliplr(rings')+1)./2);
%                         end
                    end;
                 case {'8 bars with blanks Checks Together (90) 2.5d/s','8 bars with blanks Checks Together (90) 5d/s'};
                    ...
                    for ii=1:numMotSteps,
                        if sineChecks==1
                            tmprings1 = (((cos(y*numSubRings*(2*pi/ringWidth)+(ii-1)/numMotSteps*2*pi)+1)/2));
                            tmprings2 = (((cos(y*numSubRings*(2*pi/ringWidth)+(ii-1+numMotSteps/2)/numMotSteps*2*pi)+1)/2));
                        else
                            tmprings1 = sign(2*round((cos(y*numSubRings*(2*pi/ringWidth)+(ii-1)/numMotSteps*2*pi)+1)/2)-1);
                            tmprings2 = sign(2*round((cos(y*numSubRings*(2*pi/ringWidth)+(ii-1+numMotSteps/2)/numMotSteps*2*pi)+1)/2)-1);                            
                        end
                        rings(posWedges) = tmprings1(posWedges);
                        rings(negWedges) = tmprings2(negWedges);
...
%                         else
%                             checks(:,:,ii)=(fliplr(wedges').*fliplr(rings')+1)./2;%minCmapVal+ceil((maxCmapVal-minCmapVal) * (fliplr(wedges').*fliplr(rings')+1)./2);
%                         end

                    end;
                case {'8 bars with blanks Sine (90) 1.5d/s', '8 bars with blanks Sine (90) 2.5d/s', '8 bars with blanks Sine (90) 3.75d/s', '8 bars with blanks Sine (90) 5d/s', '8 bars with blanks Sine (90) 7.5d/s', '8 bars with blanks Sine (90) 1.5d/s 20%','8 bars with blanks Sine (90) 2.5d/s 20%','8 bars with blanks Sine (90) 3.75d/s 20%', '8 bars with blanks Sine (90) 5d/s 20%','8 bars with blanks Sine (90) 7.5d/s 20%'};
...
%                         end
                    ...
                    %Set images to ones for making stimulus file for model
                    %checks=ones(size(checks));
                    
                case {'8 bars with blanks Sine (0) 2.5d/s',  '8 bars with blanks Sine (0) 5d/s'};
                    ...
                    for ii=1:numMotSteps,
                        if sineChecks==1
                            tmprings1 = (((cos(x*numSubRings*(2*pi/ringWidth)+(ii-1)/numMotSteps*2*pi)+1)/2));
                            tmprings2 = (((cos(x*numSubRings*(2*pi/ringWidth)+(ii-1)/numMotSteps*2*pi)+1)/2));
                        else
                            tmprings1 = sign(2*round((cos(y*numSubRings*(2*pi/ringWidth)+(ii-1)/numMotSteps*2*pi)+1)/2)-1);
                            tmprings2 = sign(2*round((cos(y*numSubRings*(2*pi/ringWidth)+(ii-1)/numMotSteps*2*pi)+1)/2)-1);                            
                        end
...
%                         end
        ...
        switch params.experiment
            case {'8 bars','8 bars with blanks','8 bars (slow)','8 bars with blanks (attn)','8 bars with blanks (lr)','8 bars with blanks (lr 2)', '8 bars with blanks (lr 3)', '8 bars with blanks Checks Counter (90) 2.5d/s', '8 bars with blanks Checks Counter (90) 5d/s', '8 bars with blanks Checks Together (90) 2.5d/s','8 bars with blanks Checks Together (90) 5d/s', '8 bars with blanks Sine (90) 1.5d/s', '8 bars with blanks Sine (90) 2.5d/s', '8 bars with blanks Sine (90) 3.75d/s', '8 bars with blanks Sine (90) 5d/s', '8 bars with blanks Sine (90) 7.5d/s', '8 bars with blanks Sine (0) 2.5d/s', '8 bars with blanks Sine (0) 5d/s', '8 bars with blanks Sine (90) 1.5d/s 20%','8 bars with blanks Sine (90) 2.5d/s 20%','8 bars with blanks Sine (90) 3.75d/s 20%', '8 bars with blanks Sine (90) 5d/s 20%','8 bars with blanks Sine (90) 7.5d/s 20%'}
                if softEdgeWindow>0
...
fullTR=params.tr./duration.stimframe;
numTRs=length(sequence)/fullTR;
motionSeq=[];
for tmpcounter=1:numTRs
    if rand(1)<0.5
        if rand(1)<0.5  
            motionSeqAdd=vertcat(ones(halfTR,1), -1.*ones(fullTR-halfTR,1));
        else
            motionSeqAdd=vertcat(ones(fullTR-halfTR,1), -1.*ones(halfTR,1));
        end
    else
        if rand(1)<0.5  
            motionSeqAdd=vertcat(-1.*ones(halfTR,1), ones(fullTR-halfTR,1));
        else
            motionSeqAdd=vertcat(-1.*ones(fullTR-halfTR,1), ones(halfTR,1));
        end
    end
    motionSeq=[motionSeq;motionSeqAdd];
end
motionSeq = cumsum(sign(motionSeq));
