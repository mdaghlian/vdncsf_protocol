function stimulus = makeRetinotopyStimulus_checkLeftStillRight(params)
...
%    disp(sprintf('[%s]:size stimulus: %dx%d pixels.',mfilename,n,m));
    ...
    ringsStill = sign(2*round((sin(r*numSubRings*(2*pi/ringWidth))+1)/2)-1);
    
    checks   = zeros(size(rings,1),size(rings,2),params.temporal.motionSteps*3);
    tmprings3 = sign(2*round((sin(r*numSubRings*(2*pi/ringWidth)+0/numMotSteps*2*pi)+1)/2)-1);
    tmprings4 = sign(2*round((sin(r*numSubRings*(2*pi/ringWidth)+0/numMotSteps*2*pi)+1)/2)-1);
    ringsStill(posWedges)=tmprings3(posWedges);
    ringsStill(negWedges)=tmprings4(negWedges);   
    ...
    for ii=1:numMotSteps,
        ...
        checks(:,:,ii)=minCmapVal+ceil((maxCmapVal-minCmapVal) * (wedges.*ringsStill+1)./2);
        checks(:,:,ii+numMotSteps)=minCmapVal+ceil((maxCmapVal-minCmapVal) * (wedges.*ringsStill+1)./2);
        
        checkTmp=minCmapVal+ceil((maxCmapVal-minCmapVal) * (wedges.*rings+1)./2);
        checks(:,1:(size(rings,2)/2),ii)=checkTmp(:,1:(size(rings,2)/2));
        checks(:,1+(size(rings,2)/2):end,ii+numMotSteps)=checkTmp(:,1+(size(rings,2)/2):end);
        checks(:,:,ii+numMotSteps*2)=checkTmp;
    end;
    numMotSteps=numMotSteps*3;
...
%onOffSequence=(duration.cycle.stimframes/4):(duration.cycle.stimframes/4):length(sequence);

sequenceIndices=[1 params.temporal.motionSteps+1 1+params.temporal.motionSteps*2];
onSequence=[3 1 2 3 2 1];
onSequence=sequenceIndices(onSequence);
onOffSequence=[10 20 10 20 10 20 10 20 10 20 10 20];

blockLength=(onOffSequence(1)+onOffSequence(2))*(params.temporal.motionSteps*params.temporal.frequency);
sequence=repmat(onSequence, blockLength, 1);

%onOffSequence=[10 20 10 20 10 20 10 20 10 20];
onOffSequence=onOffSequence.*(params.temporal.motionSteps*params.temporal.frequency);
onOffSequence = repmat(onOffSequence(:),params.ncycles,1);
onOffSequence=cumsum(onOffSequence);
sequence = repmat(sequence(:),params.ncycles,1);
...
for insertCounter=2:2:length(onOffSequence)
    if insertCounter==2
        sequence((onOffSequence(1)+1):onOffSequence(2))=1;%(size(images,3)-1)*0.75+1;
    else
        sequence((onOffSequence(insertCounter-1)+1):onOffSequence(insertCounter))=1;%(size(images,3)-1)*0.75+1;
    end
end
