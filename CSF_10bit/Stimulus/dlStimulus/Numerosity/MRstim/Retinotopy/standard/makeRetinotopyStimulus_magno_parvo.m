function [stimulus otherSequence] = makeRetinotopyStimulus_magno_parvo(params)
...
%                     
%                                 end;
%                                 for ii=2:numMotSteps/2
%                                     contrast=abs(1-(ii-1)/(numMotSteps/2)./0.5);
%                                     if (ii-1)/(numMotSteps/2)==0.5;
%                                         checks(:,:,ii)=((checks(:,:,1)-0.5).*0)+0.5;
%                                     elseif (ii-1)/(numMotSteps/2)<0.5;
%                                         checks(:,:,ii)=((checks(:,:,1)-0.5).*contrast)+0.5;   
%                                     elseif (ii-1)/(numMotSteps/2)>0.5;
%                                         checks(:,:,ii)=((checks(:,:,1+numMotSteps/2)-0.5).*contrast)+0.5; 
%                                     end
%                                 end
% 
% 
%                                 checks(:,:,2+numMotSteps/2:numMotSteps)=flipdim(checks(:,:,2:numMotSteps/2), 3);    
% 
%                                 if isfield(params, 'contrast')
%                                     checks=((checks-0.5).*params.contrast
%                                     )+0.5;
%                               end
                    ...
                    %only valid if size(rings,1)=size(rings,2)
...
%                         checks(:,:,ii)=newmat;
%                     end
                    %[checkfactor,checks(:,:,ii)]=rmsscale(checks(:,:,ii),0.3);
                        
                                        
                  %This is the contrast bit
                    for ii=1:numMotSteps
                        
                        contrast=sin((ii-1)*2*pi/numMotSteps);
                        checks(:,:,ii)=(checksIn-.5).*contrast + 0.5; 
                    end
                        
                    %rms normalizing
%                     [checkfactor,checks]=rmsscale(checks,0.3);
                        
                        
                        
                        
%                         contrast=abs(1-(ii-1)/(numMotSteps/2)./0.5);  %<--now a lin table, make it sine wave
%                         if (ii-1)/(numMotSteps/2)==0.5;
%                             checks(:,:,ii)=((checks(:,:,1)-0.5).*0)+0.5;
%                         elseif (ii-1)/(numMotSteps/2)<0.5;
%                             checks(:,:,ii)=((checks(:,:,1)-0.5).*contrast)+0.5;   
...
%                         end
                    ...
                    %rescaled by max value, different way might be
                    %neccessary.
...
% numberOfOnes=floor(fullTR/numMotSteps)*numMotSteps;
% numberOfZeros=fullTR-numberOfOnes;
% 
% motionSeq=(repmat([zeros(numberOfZeros,1); ones(numberOfOnes,1)],[1 length(sequence)/fullTR]));

%motionSeq=motionSeq(:);
motionSeq=ones(size(sequence));
zeroPoint=(fullTR+1):fullTR:length(sequence);
motionSeq(zeroPoint)=0;
