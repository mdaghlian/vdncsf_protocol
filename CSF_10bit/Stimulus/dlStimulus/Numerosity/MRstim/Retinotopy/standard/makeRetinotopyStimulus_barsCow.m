function [stimulus otherSequence] = makeRetinotopyStimulus_barsCow(params)
...
%    disp(sprintf('[%s]:size stimulus: %dx%d pixels.',mfilename,n,m));
    ...
    stimSize=n+ceil(framesDuration*speedPixPerFrame*2);
...
%                images(:,:,[half+1:numMotSteps]+start) = repmat(uint8(img2),[1 1 numMotSteps-half]);
            ...
            case {'8 bars with blanks Cow(0)'} 
                tmpvar = uint8(zeros(m,n));
                tmpvar(window) = 1;
                tmpvar = repmat(tmpvar,[1 1 framesDuration]);
                window = tmpvar == 1;
                img         = uint8(bk*ones(size(checks)));
                img(window) = checks(window);
                images(:,:,(imgNum-1).*framesDuration+1:imgNum.*framesDuration) = img;
        end
