function stimulus = makeRetinotopyStimulus_FlickerBH(params)
...
%     myim = myim.*mask;
    ...
    myimtmp=zeros(size(myim, 1), size(myim, 2), length(params.flickerContrast));
    for contrastcounter=1:length(params.flickerContrast)
        myimtmp(:,:,contrastcounter) = myim.*params.flickerContrast(contrastcounter);
    end
    myim=myimtmp;
    
    
    images   = uint8(ones(size(rings,1),size(rings,2),2*numMotSteps/min(params.flickerFrequencies)+1).*128);
...
%         rings(negWedges)=tmprings2(negWedges);
        ...
        images(:,:,ii)= uint8(minCmapVal+ceil((maxCmapVal-minCmapVal) .* (myim(:,:,1) .* contrasts(ii)+1)./2));
        fprintf('.');drawnow;
    end;
    for ii=numel(contrasts)+1:numel(contrasts)*2,
%         tmprings1 = sign(2*round((sin(r*numSubRings*(2*pi/ringWidth)+(ii-1)/numMotSteps*2*pi)+1)/2)-1);
%         tmprings2 = sign(2*round((sin(r*numSubRings*(2*pi/ringWidth)-(ii-1)/numMotSteps*2*pi)+1)/2)-1);
%         rings(posWedges)=tmprings1(posWedges);
%         rings(negWedges)=tmprings2(negWedges);

        images(:,:,ii)= uint8(minCmapVal+ceil((maxCmapVal-minCmapVal) .* (myim(:,:,2) .* contrasts(ii-numel(contrasts))+1)./2));
        fprintf('.');drawnow;
    end;
    fprintf('Done.\n');
end;
...
% main wedges/rings
sequence = (duration.stimframe:duration.stimframe:duration.cycle.seconds/2)'*repmat(params.flickerFrequencies,1,params.ncycles);
...
sequence(:,length(params.flickerContrast):length(params.flickerContrast): size(sequence,2))=sequence(:,length(params.flickerContrast):length(params.flickerContrast): size(sequence,2))+(length(params.flickerContrast)-1)*numel(contrasts);
sequence = sequence(:);
